                                ...
                                <div class="row">
                                  ...
                                  <div class="col-sm-4">
                                    <div class="position-relative p-3 bg-blue mt-1 my-2" style="height: 180px">
                                      ...
                                      <button type="button" id="detail_klinik" value="{{$klinik->nama_klinik}}" class="btn btn-default showmodal" >
                                        Lihat Detail
                                      </button>
                                    </div>
                                    <div class="modal fade" id="editModal">
                                      <div class="modal-dialog modal-lg">
                                        ...
                                        <table class="table table-bordered">
                                          ...
                                          <tbody>
                                            <div class="h4" id="tujuan_susah">asd</div> ->where('role', '0')->where('klinik_tujuan', $klinik->nama_klinik)->where('tanggal_reservasi', $today)
                                            @foreach ($users->where('role', '0')->where('tanggal_reservasi', $today) as $user)
                                            <tr>
                                              <td>{{$user->no_antrian+1}}</td>
                                              <td>{{$user->name}}</td>
                                              <td><button type="button" id="{{$user->id}}" value="{{$user->id}}" class="editbtn btn btn-default">Lanjutkan</button></td>
                                            </tr>
                                            @endforeach
                                          </tbody>
                                        </table>
                                          ...
                                          <div class="modal-footer justify-content-between">
                                            ...
                                            <button type="submit" class="btn  btn-outline-success">Lanjut Antrian</button>
                                          </div>
                                        </div>
                                        <!-- /.modal-content -->
                                      </div>
